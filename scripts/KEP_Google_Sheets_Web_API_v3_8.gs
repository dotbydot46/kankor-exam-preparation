/**
 * KEP v3.8 — Google Sheets Web API for Review + Published Practice + Library
 * ------------------------------------------------------
 ...
 * - Returns Published books, notes, PDFs, and past papers to the Library page.
 *
 * Setup:
...
function doGet(e) {
  try {
    ...
    } else if (action === 'publishedresources' || action === 'library' || action === 'resources') {
      payload = { ok: true, generatedAt: new Date().toISOString(), resources: getPublishedResources_() };
    } else {
...
function classifySheet_(headers, sheetName) {
  ...
  if (sheetName.toLowerCase().includes('approved past papers')) return 'Past Paper';
  if (sheetName.toLowerCase().includes('approved books')) return 'Book / Notes';
...
function getPublishedResources_() {
  const ss = SpreadsheetApp.openById(KEP_LIVE_CONFIG.spreadsheetId);
  const output = [];
  ...
  ss.getSheets().forEach(sheet => {
    ...
    let headers = values[0].map(h => String(h).trim());
    const type = classifySheet_(headers, sheet.getName());
    if (type !== 'Past Paper' && type !== 'Book / Notes') return;
    ...
    values.slice(1, KEP_LIVE_CONFIG.maxRowsPerSheet + 1).forEach((row, idx) => {
      ...
      const status = getCell_(headers, row, 'Review Status') || getCell_(headers, row, 'Status') || getCell_(headers, row, 'Publish?');
      const normalizedStatus = String(status).trim().toLowerCase();
      if (!['published', 'publish', 'yes', 'approved'].includes(normalizedStatus)) return;

      const mapped = mapRowToLibraryResource_(type, headers, row, sheet.getName(), idx + 2);
...
function mapRowToLibraryResource_(type, headers, row, sheetName, rowNumber) {
  const getAny = labels => {
    for (const label of labels) {
  ...
  const isPaper = type === 'Past Paper';
  const title = isPaper
    ? getAny(['Paper title','Title','Paper Title'])
    : getAny(['Resource title','Title','Resource Title']);

  const fileUrl = getAny(['File link (Google Drive / Dropbox / other)','File URL','PDF URL','URL','Link']);
  if (!title && !fileUrl) return null;

  const subject = getAny(['Subject']) || 'General';
  const language = getAny(['Language']) || 'Not specified';
  const year = getAny(['Year']) || '';
  const grade = getAny(['Grade / class','Grade','Class']) || '';
  const resourceType = isPaper
    ? getAny(['Paper type','Type']) || 'Past paper'
    : getAny(['Resource type','Type']) || 'Book / Notes';

  const sourceType = getAny(['Source type','Source Type']) || 'Reviewed source';
  const sourceDetails = getAny(['Source details / permission note','Source details / book page / paper year / link','Source Details','Notes']) || '';
  const notes = getAny(['Notes','Reviewer Notes','Review Notes']) || '';

  return {
    id: `${isPaper ? 'PAPER' : 'BOOK'}-${rowNumber}`,
    type,
    title: title || (isPaper ? 'Published past paper' : 'Published resource'),
    subject,
    language,
    year,
    grade,
    resourceType,
    sourceType,
    sourceDetails: shorten_(sourceDetails, 140),
    notes: shorten_(notes, 160),
    fileUrl,
    sheetName,
    rowNumber,
    origin: `Live Google Sheet: ${sheetName}, row ${rowNumber}`
